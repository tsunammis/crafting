...
/*
Sometimes we’d like our Go programs to intelligently handle Unix signals. For
example, we might want a server to gracefully shutdown when it receives a
SIGTERM, or a command-line tool to stop processing input if it receives a
...
func main() {
	...
	// We’ll create a channel to receive these notifications (we’ll also make
	// one to notify us when the program can exit).
	...
	// indicated by the goroutine above sending a value on done) and then exit.
